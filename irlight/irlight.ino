/*
 * irlight led lighting effects for FastLED.
 * 
 *       By: Andrew Tuline
 *    Date: January, 2017
 *      URL: www.tuline.com
 ...
 * you already be able to download, install and use 3rd party libraries.
 * 
 * ********************************************************************************************************************************************************
 * 
 * 
 ...
 * - WS2812 strands ONLY work with the demo mode. They will not support IR functionality.
 ...
 * Startup Operation
 * 
 * irlight starts up in demo mode by default and is initially configured for a maximum of 100 APA102 LED's. Before using any strands, you will want to configure your strand length.
 * 
 * 
 * For normal operation:
 * 
 * - Configure and compile irlight for your type of strand, and pins used (WS2812's don't work with IR).
 * - Use the IR controller to modify and save the strand length (up to 100) to support your actual strand.
...
bool demorun = 1;                                             // 1 = demo mode, 0 = regular mode.
...
void demo_check(){
  ...
  if(demorun) {                                                   // Is the demo flag set? If so, let's cycle through them.
    uint8_t secondHand = (millis() / 1000) % (maxMode*10);        // Adjust for total time of the loop, based on total number of available modes.
    static uint8_t lastSecond = 99;                               // Static variable, means it's only defined once. This is our 'debounce' variable.
    if (lastSecond != secondHand) {                               // Debounce to make sure we're not repeating an assignment.
      lastSecond = secondHand;
        if(secondHand%10==0) {                                     // Every 10 seconds.
          ledMode = secondHand/10;
...
//----------------- IR Receiver and Command Processing ---------------------------------------------
